...
import {
	fetchAllByEmployee,
	createOne as createOnePeriod,
	update as updatePeriod,
} from '../../assets/FetchServices/TimeOffHistory';
...
export default function TimeOffRequest({ close, sendRequest, initialRequest, style }) {
	...
	const [from, setFrom] = useState(initialRequest ? initialRequest.from : dayjs().toDate());
	const [to, setTo] = useState(initialRequest ? initialRequest.to : dayjs().toDate());
	...
	useEffect(() => {
		setCategory(
			initialRequest ? categoryMenu.filter(item => item.timeOffId === initialRequest.timeOffId)[0] : categoryMenu[0]
		);
	}, [categoryMenu]);
			...
			if (data) {
				...
				filteredData.forEach(p => {
					policies[p.category] = {
						...
						type: p.category,
						availableHours:
							p.hoursLeft + (initialRequest && initialRequest.timeOffId === p.timeOffId ? initialRequest.hours : 0),
		...
		fetchAllByEmployee(currentUser).then(data => {
			if (data) {
				...
				data.forEach(p => {
					...
					if (
						dayjs(p.startDate).isSameOrAfter(dayjs().subtract(1, 'day')) &&
	...
	function validateDates() {
		for (const p of timeOffDates) {
			if (
				(isValidPeriod(p.from, from) && isValidPeriod(from, p.to)) ||
				...
				if (initialRequest.timeOffId !== category.timeOffId) {
					...
					const refundBalance = {
						...
						cumulativeHoursTaken:
							categoryMenu.filter(p => p.timeOffId === initialRequest.timeOffId)[0].hoursUsed - initialRequest.hours,
					};
	...
	return (
		...
		>
			...
			>
				{initialRequest ? <h3>Редактировать мой запрос на отгул</h3> : <h3>Запросить новый отгул</h3>}
				<CloseIcon
					onClick={close}
					sx={{
			...
			<Header4>Категории отгулов</Header4>
			<Select
				value={category}
			...
			>
				{categoryMenu.map(option => (
					<MenuItem value={option}>
						{option.type}: {Math.floor(option.availableHours / 8)} дней ({option.availableHours} часов)
					</MenuItem>
				))}
			</Select>
			{/*Set starting and ending dates of time off period*/}
			<Stack direction='row' alignItems='center' spacing={4} sx={{ marginBottom: '40px' }}>
				<Box>
					<Header4>От</Header4>
					<Chip
			...
			<Stack direction='row' alignItems='center' spacing={1}>
				...
				/>
				<p>Укажите количество часов для каждого дня периода отсутствия</p>
			</Stack>
			{/*Time off per day table*/}
			<MemoTimeOffTable dateRange={dateRange} eachDay={eachDay} onChange={calculateTimeOffHours} />
			{/*Time off summary*/}
			<p style={{ marginBottom: '30px' }}>
				{fullDaysOff} полных дней ({fullDaysOff * 8} часов) и {halfDaysOff} пол дня ({halfDaysOff * 4} часов) будут
				запрошенны ({totalHoursOff} часов в сумме).
			</p>
			...
			{initialRequest && initialRequest.status === 'Approved' && (
				<h4 style={{ color: '#F79009', marginBottom: '15px' }}>
					Этот запрос на отгул уже утвержден. Если вы внесете изменения, потребуется повторное утверждение.
				</h4>
			)}
			{/*Validation messages to be displayed if the request being submitted is invalid */}
			{!validDates && (
				<h4 style={{ color: '#D92D20', marginBottom: '15px' }}>
					Даты вашего запроса на отгул пересекаются с уже запланированными периодами отсутствия.
				</h4>
			)}
			{!sufficientTime && (
				<h4 style={{ color: '#D92D20', marginBottom: '15px' }}>
					Вы запрашиваете {totalHoursOff} часов отгула. В выбранной политике отгулов доступно только{' '}
					{category.availableHours} часов.
				</h4>
			)}
			{/*Error message to be displayed if an error occurs*/}
			{errorOccurred && (
				<h4 style={{ color: '#D92D20', marginBottom: '15px' }}>Произошла ошибка. Не удалось отправить запрос на отгул.</h4>
			)}
			{/*Send or cancel*/}
