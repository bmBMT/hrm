...
export default function ChooseEmployees() {
	...
	const availableEmployees = useMemo(
		() => employees?.filter(emp => emp.empId !== updatedPermissions?.[0]?.employee?.empId) || [],
	...
	const addEmployee = newEmployee => {
		if (!newEmployee || addEmployeesToManager.some(emp => emp.empId === newEmployee.empId)) {
			return;
		}
	...
	const deleteEmployee = employeeToDelete => {
		setAddEmployeesToManager(prev => prev.filter(emp => emp.empId !== employeeToDelete.empId));
	};
	...
	return (
		<Grid container spacing={2}>
			<Grid item xs={8}>
				...
				>
					Выберите сотрудников, подчинённых {currentEmployee.firstName} {currentEmployee.lastName}
				</Typography>
				<Autocomplete
					fullWidth
					color='secondary'
					size='small'
					options={availableEmployees?.sort((a, b) => a.firstName.localeCompare(b.firstName))}
					getOptionLabel={option => `${option.firstName} ${option.lastName}`}
					renderInput={params => <TextField {...params} placeholder='Select employees' />}
					sx={{
						width: '100%',
						marginBottom: '16px',
					}}
				...
				<Grid container spacing={2} sx={{ maxHeight: '100px', overflow: 'auto' }}>
