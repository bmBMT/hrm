...
export default function SetupCompanyPage({ style }) {
	...
	const steps = [
		{
			...
			description: 'Введите основные данные компании',
		},
		{
			label: 'Отделы',
			description: 'Выберите отделы компании',
		},
		{
			label: 'Роли',
			description: 'Назначьте роли сотрудников',
		},
		{
			label: 'Завершение',
			description: 'Завершите настройку системы',
		},
	];
	...
	return (
		...
		>
			...
			/>
			<CustomizedSteppers
				stepnumber={page === 'Company' ? 0 : page === 'Departments' ? 1 : page === 'Roles' ? 2 : 3}
				steps={steps}
				style={{
					margin: 'auto',
					padding: '2% 150px',
			...
			/>
			{page === 'Company' && (
				<SetupCompanyMenu advancePage={() => setPage('Departments')} style={{ margin: '40px auto' }} />
			)}
			{page === 'Departments' && (
				<SetupMenu
					item={'Departments'}
