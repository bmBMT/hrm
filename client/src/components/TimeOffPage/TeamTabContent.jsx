...
export default function TeamTabContent({ style }) {
		...
		if (isAdmin) {
			...
			fetchAll()
				.then(data => {
					...
					team.forEach(id => {
						fetchAllByEmployee(id).then(data => {
							if (data) {
								data.forEach(p => {
									periods.push({
										id: p.id,
	...
	const activeFilters = [];
	if (approvedFilter) {
		activeFilters.push('Approved');
	}
	if (waitingFilter) {
		activeFilters.push('Waiting');
	}
	if (rejectedFilter) {
		activeFilters.push('Rejected');
	}
		...
		>
			...
			>
				<Stack direction='row' alignItems='center' spacing={3}>
					<h3>История моего отдела</h3>
					<Label mode='brand' label={timeOffPeriods.length} style={{ borderRadius: '50%' }} />
				</Stack>
					...
					<MenuToggleButton
						...
						menuItems={{
							Approved: [
								approvedFilter,
								value => {
									if (activeFilters.length >= 2 || !approvedFilter) {
							...
							Waiting: [
								waitingFilter,
								value => {
									if (activeFilters.length >= 2 || !waitingFilter) {
							...
							Rejected: [
								rejectedFilter,
								value => {
									if (activeFilters.length >= 2 || !rejectedFilter) {
										setRejectedFilter(value);
									}
								},
							],
						}}
			...
			{loadingPeriods ? (
				...
				<>
					...
					{filteredPeriods.length > 10 && (
						<PagesNavBar numOfEntries={filteredPeriods.length} currentPage={currentPage} handlePage={handlePage} />
					)}
				</>
			) : (
				<>
