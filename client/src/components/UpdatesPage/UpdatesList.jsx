...
export default function UpdatesList({ updates, refresh, style }) {
	...
	function handleSwitch(up) {
		...
		update({
			...
			status:
				checkNotificationStatus(up, currentUser) === 'new' || checkNotificationStatus(up, currentUser) === 'waiting'
					? 'seen'
					: 'new',
		}).then(data => {
			if (data) {
	...
	function retrieveDetails(up) {
		...
		//Retrieve details for "New time off request" update
			...
			const details = {
				...
				timeOffBalance: up.employeeAnnualTimeOff.hoursAllowed - up.employeeAnnualTimeOff.cumulativeHoursTaken,
		...
		//Retrieve details for "Time off request approved/rejected" update
			...
			up.subject === 'Your time off request has been approved' ||
	...
	return (
		<>
					...
					<TableBody>
						{updates.map(update => (
							...
							>
								{/*Update status*/}
								<TableCell>
									{checkNotificationStatus(update, currentUser) === 'new' && (
										<Label mode='status' dot='orange' label='Новый' />
									)}
									{checkNotificationStatus(update, currentUser) === 'waiting' && (
										<Label mode='status' dot='red' label='Ожидание' />
									)}
									{checkNotificationStatus(update, currentUser) === 'seen' && (
										<Label mode='status' dot='grey' label='Просмотрено' />
									)}
								</TableCell>
								...
								<TableCell align='right' sx={{ paddingRight: 0, width: '16%' }}>
									<HRMButton mode='tertiary' onClick={() => handleSwitch(update)}>
										<b>{checkNotificationStatus(update, currentUser) === 'seen' ? 'Отменить' : 'Просмотреть'}</b>
									</HRMButton>
								</TableCell>
								{/*View button*/}
									...
									<HRMButton
										...
										onClick={() => {
											...
											if (update.subject === 'New team member added') {
												...
												update.subject === 'Your time off request has been approved' ||
			...
			<Dialog open={requestResolved} onClose={() => setRequestResolved(false)}>
				<TimeOffRequestResolved time_off_information={requestResolvedDetails} close={() => setRequestResolved(false)} />
			</Dialog>
			...
			<Dialog open={requestDeleted} onClose={() => setRequestDeleted(false)}>
				<TimeOffDeleted request_information={requestDeletedDetails} close={() => setRequestDeleted(false)} />
			</Dialog>
			{/*Time off period not deleted popup component*/}
			<Dialog open={requestNotDeleted} onClose={() => setRequestNotDeleted(false)}>
				<TimeOffNotDeleted request_information={requestNotDeletedDetails} close={() => setRequestNotDeleted(false)} />
			</Dialog>
		</>
	);
}
